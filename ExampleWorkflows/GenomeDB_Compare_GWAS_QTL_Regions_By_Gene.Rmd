...
display_qtl <- function (gene, tissues, db) {
    ...
    viz <- ggplot(qtls, aes(x = position, y = -log10(pvalue))) +
        geom_point(aes(alpha = sqrt(1 / (pvalue + 1e-50))),
                   colour = "darkblue") +
        facet_wrap(~ smts + gene_symbol, drop = TRUE) +
        ylab("-log10( pvalue )") + xlab(sprintf("CHR%s position (MB)", unique(qtls$chromosome))) +
        guides(size = "none", alpha = "none", shape = "legend") +
        theme_minimal()
...
Going to play around with this and try to make these graphs interactive :)
...
Gosh that is cool - and so easy.

Below I add a 'z' dimension in the aesthetics, which will let me include this value in the tooltip. This is quite a cool, nifty trick
...
display_qtl <- function (gene, tissues, db) {
    ...
    viz <- ggplot(qtls, aes(x = position, y = -log10(pvalue), z = gene_symbol)) +
        geom_point(aes(alpha = sqrt(1 / (pvalue + 1e-50))),
                   colour = "darkblue") +
...
ggplotly(display_qtl(genes, tissues, db), tooltip = c("z", "x", "y"))
```

Now, let's try to add hte GWAS plot to this:
...
display_gwas <- function (feature, trait, db) {
    ...
    viz <- ggplot(gwas, aes(x = position, y = -log10(pvalue), z = rsid)) +
        geom_point(colour = "darkviolet", alpha = 0.3) +
        facet_wrap(~ dataset) +
        ylab("-log10( pvalue )") + xlab("") +
        theme_minimal()
...
ggplotly(display_gwas("TCF7L2", "diabetes", db), tooltip = c("z", "x", "y"))
```

This is great, I had to add rsid to the genes data as well, otherwise it spat the dummy rying to create the tooltips.

AM going to now try to layer these two plots:

```{r}
p <- subplot(
  
  ggplotly(display_qtl(genes, tissues, db), tooltip = c("z", "x", "y")),
  ggplotly(display_gwas("TCF7L2", "diabetes", db)),
  nrows = 2
)
p
```
...
There are now only two things I need to do:  

  1. add RSIDs to the qtl data  
  2. try to figure out if I can link between each plot (i.e. on hover, highlight the same rsids in all)  
  
Turns out (1) isn't so straightforward. The QTL data is far more dense than the GWAS. dim_snp was populated from the GWAS data, so there is very little overlap in these two. One solution might be to simply draw a vertical line that matches up to the cursor position. See: htps://plot.ly/r/shiny-coupled-hover-events/
