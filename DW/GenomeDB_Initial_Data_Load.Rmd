...
reset_staging <- function (conn) {
    sqlQuery(conn, "truncate table stage.qtl;")
}
...
```{r}
setwd("G:/Datasets/GenomeDBData/filtered_sets")
library(RODBC)
...
conn <- odbcConnect("R-SQL")
sqlQuery(conn, "use GenomeDB;")
dim_tissue <- sqlQuery(conn, "select *, LEFT(smtsd, 18) as 'stub' from GenomeDB.dbo.dim_tissue;")
dim_tissue$stub <- gsub(" - ", " ", dim_tissue$stub)
odbcClose(conn)

for (qtl_ in qtl_files[4]) {
